...
{
	printk("JH: %s:: This will notify rx packet is available\n",__func__);
	virt_mb();
...
static u32 ivshm_net_tx_advance(struct ivshm_net_queue *q, u32 *pos, u32 len)
{
	...
	printk("JH: %s:: Before transmitting a packet\n",__func__);
...
static int ivshm_net_tx_frame(struct net_device *ndev, struct sk_buff *skb)
{
	...
	printk("JH: %s:: Constructing Tx frame\n",__func__);
...
static void ivshm_net_state_change(struct work_struct *work)
{
	...
	printk("JH: %s:: Ivshmem net state has changed\n",__func__);
	...
	case IVSHM_NET_STATE_RESET:
		 ...
		 * to INIT.
		 */
		printk("JH: %s:: Ivshmem net state RESET\n",__func__);
	...
	case IVSHM_NET_STATE_INIT:
		 ...
		 */
		printk("JH: %s:: Ivshmem net state INIT\n",__func__);
	...
	case IVSHM_NET_STATE_READY:
		 ...
		 * RUN.
		 */
		printk("JH: %s:: Ivshmem net state READY\n",__func__);
		if (rstate >= IVSHM_NET_STATE_READY) {
	...
	case IVSHM_NET_STATE_RUN:
		 ...
		 */
		printk("JH: %s:: Ivshmem net state RUN\n",__func__);
		if (rstate == IVSHM_NET_STATE_RESET) {
			   ...
			   const struct pci_device_id *id)
{
	...
	printk("JH: %s:: IVSHMEM net probe triggered\n",__func__);
...
static const struct pci_device_id ivshm_net_id_table[] = {
	{ PCI_DEVICE(PCI_VENDOR_ID_REDHAT_QUMRANET, 0x1110),
		...
		/*(PCI_CLASS_OTHERS << 16) | (0x01 << 8), 0xffff00 },*/	/*JH original*/
	{ 0 }
};
